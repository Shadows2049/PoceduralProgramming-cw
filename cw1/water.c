...
// YOU WILL ALSO USE THIS SECTION TO INITIALISE ANY VARAIABLES THAT REQUIRE PROPER INITIALISATION
   ...
   double QTotalCommercial;
   double QDomesticCost;
   double QCommercialCost;
   double QPD;
   double QPC;
   double QBasicCharge;
   int n;
...
if(CustomerType == 1){
 ...
 if(-1<QWaterConsumption,QWaterConsumption<6){
   	QFreshWaterCharge = (QWaterConsumption * 0.2);


   }

   else if(5<QWaterConsumption,QWaterConsumption<13){
   	QFreshWaterCharge = ((QWaterConsumption-5) * 0.35) + 1;

   }


  else if(12<QWaterConsumption,QWaterConsumption<26){
   	QFreshWaterCharge = ((QWaterConsumption-12) * 0.5) + 1 + 2.45;

   }


  else if(25<QWaterConsumption,QWaterConsumption<41){
   	QFreshWaterCharge = ((QWaterConsumption-25) * 0.75) + 1 + 2.45 + 6.5;

   }


   else if(QWaterConsumption>40){
   	QFreshWaterCharge = ((QWaterConsumption-40) * 2.5) + 1 + 2.45 + 6.5 + 11.25;

   }

   QSurfaceWaterCharge = 10;
   QStandingCharges = 91 * 0.1;
   QTotalCharge = QFreshWaterCharge + QWasteWaterCharge + QSurfaceWaterCharge + QStandingCharges;
   QAmountToPay = QTotalCharge ;
   QPD = QPD + QTotalCharge;
   QTotalDomestic = QTotalDomestic + QWaterConsumption ;
   QDomesticCost = QTotalDomestic;

   if(QTotalCharge > QMaxDomesticBill){
    QMaxDomesticBill = QTotalCharge;
   }
    n=n+1;
    QAverageDomesticBill = (QPD/n);
     ...
     QStandingCharges = 91 * 1.3;
     QVAT =  0.2 * (QFreshWaterCharge + QWasteWaterCharge + QSurfaceWaterCharge + QStandingCharges);
     QAmountToPay = QFreshWaterCharge + QWasteWaterCharge + QSurfaceWaterCharge + QStandingCharges + QVAT;
     QTotalCharge = QFreshWaterCharge + QWasteWaterCharge + QSurfaceWaterCharge + QStandingCharges;


     QPC = QPC + QTotalCharge;
     QTotalCommercial = QTotalCommercial + QWaterConsumption;
     QCommercialCost = QTotalCommercial;
   ...
   QTotalFreshWater = QTotalCommercial + QTotalDomestic;
   QRevenue = QPD + QPC;
   QCost = QCommercialCost + QDomesticCost;
